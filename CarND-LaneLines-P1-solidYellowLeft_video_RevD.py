...
def draw_line_polyfit(img, x, y, previous_lines, isChallengeVideo=False, slope_threshold=0.05, intercept_threshold=50.0, color=[255, 0, 0], thickness=6):
    if len(x) == 0: 
        return
    
    # Least squares polynomial fit (degree = 1 --> straight line)
    p = np.polyfit(x, y, 1)
    slope = p[0]
    intercept = p[1]
    ...
    y2 = int(0.63*img.shape[0]) if isChallengeVideo == True else int(0.60*img.shape[0])
    x2 = int((y2-intercept)/slope)
    
    # Check slope with previous lines
    if len(previous_lines) > 0:
        # Compute mean for slope and intercept
        mean = np.mean(previous_lines, axis=0)
        # Compute slope/intercept difference between the current and previous lines
        slope_diff = abs(slope - mean[0])
        intercept_diff = abs(intercept - mean[1])
        # In case slope/intercept difference is too big, get the mean of stored lines parameters
        if (slope_diff > slope_threshold) or (intercept_diff > intercept_threshold):
            slope = previous_lines[-1][0]
            intercept = previous_lines[-1][1]
            # Correct line points
            x1 = int((y1-intercept)/slope)
            x2 = int((y2-intercept)/slope)
            
    previous_lines.append([slope, intercept])
    # Draw line
    cv2.line(img, (x1, y1), (x2, y2), color, thickness)
    # Return current line parameters
...
def draw_lines(image):
    ...
    lines = hough_lines_process(image, rho, theta, threshold, min_line_length, max_line_gap)
    slope_boundaries = [0.5, 1.0]
    separate_points = separate_left_and_right_lines(lines, slope_boundaries) 
    ...
    lines_edges = cv2.addWeighted(image, 1, lines_img, 1, 0)
    return lines_edges
